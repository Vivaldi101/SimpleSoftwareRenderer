...
void R_CalculateLighting(const RendererBackend *rb, AmbientState as, Vec3 camera_dir, Vec3 camera_pos) {
	Vec4 base = {};
	Vec4 total = {};
	int num_polys = rb->vis_num_polys;
   ...
   Light *lights = rb->lights;
	for (int i = 0; i < num_polys; ++i) {
      Vec4 color = {};
		...
		PolyVert v1 = rb->vis_polys[i].vertex_array[1];
		PolyVert v2 = rb->vis_polys[i].vertex_array[2];
		...
		for (int j = 0; j < num_lights; ++j) {
			...
			r32 spot = (lights[j].flags & SPOT_LIGHT) ? ((r32)pow(MAX(factor, 0.0f), 32)) : 1.0f;
			...
			// specular component
			Vec3 h = l + view;
			dot = (r32)pow(MAX(Dot3(n, h), 0.0f), 3);
